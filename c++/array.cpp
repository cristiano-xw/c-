...
public:
	...
	virtual ~Shape() { }
	bool operator >(Shape & shape); //重载比较预算符   
...
void insertSort(Array<Shape *> &a, int index, Shape * ptr) 
{// 将新创建的形状ptr插入到动态数组a中
	int size = a.getSize();
	int boundary = 0;
...
int main () 
{
	...
	Array<Shape *> a(preSize);  //在定义的时候同时初始化
	for (int i = 0; i < preSize; i++)	// 要将a中的指针元素初始化为NULL，否则有可能是“野指针”
		a[i] = NULL;
